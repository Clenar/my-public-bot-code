Roadmap v11.13: План Разработки Бота @Sprofy_bot
(Последнее обновление: 16 мая 2025 г., Интеграция актуального прогресса с v11.8 как основой. Завершены этапы отладки AI-промптов, обогащения услуг, реализована иерархическая логика выбора услуг, включая callback-хэндлеры. Выявлена и добавлена задача по исправлению парсинга ответа AI при подтверждении города.)
...
✅ (v11.9) Полностью отлажена и протестирована базовая реализация: движок в папке BehaviorEngine/, читающий YAML из БД (parser.py + кэш), управляющий состоянием через БД (state_manager.py + модель UserStates), выполняющий шаги сценария (executor.py с корректной обработкой флага _internal_on_entry_actions_done и флага process_only_on_entry, форматированием параметров действий, обработкой handler_initiated_scenario_switch) и интегрированный в основной цикл (engine.py с циклической обработкой on_entry для внутренних переходов и явным commit). (Эволюция и завершение пункта из v11.6, детализировано v11.8, подтверждено v11.9)
...
Phase 4: Разработка Движка Сценариев и Реализация Базовых Сценариев (✅ Ключевые этапы завершены, идет реализация и отладка иерархического выбора услуг)
...
✅ Шаг 3: Разработка и Тестирование "Движка Сценариев" (BehaviorEngine): (Полностью выполнено и отлажено v11.9, комментарии из v11.8 подтверждают завершенность этого этапа)
...
✅ (v11.13) Реализация Сценария Регистрации Мастера (master_registration_v1.yaml):
...
✅ Создан Python-файл handlers/registration_logic.py.
✅ Реализованы хэндлеры: prepare_city_confirmation, handle_city_confirmation_callback, reset_user_state_handler, get_service_children.
...
✅ Доработан хэндлер analyze_and_match_services_initial: Реализовано обогащение matched_services_info данными из БД. (Выполнено v11.11)
✅ Реализован хэндлер prepare_service_suggestions_message с логикой "авто-детализации" и отображением подуслуг с кнопками. (Выполнено v11.12)
✅ Реализованы callback-хэндлеры для выбора услуг: handle_detail_category, handle_toggle_sub_service, handle_category_done, handle_skip_top_service, handle_add_direct_service. (v11.12 - код для них предоставлен, базовая логика протестирована, требуется полное тестирование).
🚧 Проблема с обработкой ответа AI при подтверждении города: (Выявлено v11.12)
...
🚧 Тестирование и Отладка Иерархического Выбора Услуг:
Задача: Провести комплексное тестирование всего сценария выбора услуг с различными пользовательскими вводами и последовательностями нажатия кнопок. Убедиться в корректной работе "авто-детализации", обработке всех callback'ов, обновлении state_context и правильности переходов.
...
Где мы сейчас: (Обновлено v11.13)

Фазы 0, 1, 2, 3 завершены.
Фаза 4 в активной разработке.
✅ Разработка BehaviorEngine и интеграция /start полностью завершены и отлажены.
✅ Сценарий регистрации мастера:
✅ Базовые шаги (город, первоначальный запрос услуг, классификация AI, обогащение услуг из БД) успешно реализованы и ключевые проблемы отлажены.
✅ Хэндлер prepare_service_suggestions_message и все связанные callback-хэндлеры для иерархического выбора услуг (handle_detail_category, handle_toggle_sub_service, handle_category_done, handle_skip_top_service, handle_add_direct_service) реализованы с логикой авто-детализации.
✅ Необходимые инструкции и YAML для этого этапа обновлены.
🚧 Выявлена проблема с парсингом ответа AI при подтверждении города в prepare_city_confirmation (AI добавляет префикс к CITY_FOUND:).
🚧 Текущая задача:
Исправить баг в prepare_city_confirmation.
Провести комплексное тестирование и отладку всего цикла иерархического выбора услуг, включая корректность обновления state_context и правильность переходов между различными ситуациями выбора.
Непосредственный следующий шаг: (Обновлено v11.13)

Исправить prepare_city_confirmation в handlers/registration_logic.py для более надежного парсинга ответа AI о городе.
Провести полное тестирование и отладку всего функционала иерархического выбора услуг, реализованного в prepare_service_suggestions_message и связанных с ним callback-хэндлерах.
После успешной отладки выбора услуг, приступить к реализации следующего шага в сценарии регистрации мастера (например, запрос опыта — REG_MASTER_ASK_EXPERIENCE).
🗂️ Файлы для Контекста Нового Чата (Инструкция для AI) (Обновлено v11.13)
(Список файлов сохранен из v11.8, актуализированы комментарии к изменяемым файлам).

ROADMAP.md (Этот файл): План, статус, инструкции по взаимодействию (v11.13).
PROJECT_SUMMARY.md
run.py
config.py
database/models.py
...
handlers/registration_logic.py: (Актуальная версия, содержащая все реализованные хэндлеры для регистрации, включая иерархический выбор услуг, и требующая исправления в prepare_city_confirmation).
